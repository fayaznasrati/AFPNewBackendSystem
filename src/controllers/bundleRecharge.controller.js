...
class rechargeController {
    ...
    processBundleRecharge = async (data) => {
            try {
    ...
    getPendingBundleRechargeList = async (req, res) => {
        try {
            ...
            // console.log('recharge/getPendingBundleRechargeList',JSON.stringify(req.body), JSON.stringify(req.query))
            // limit offset
            ...
            console.error('getPendingBundleRechargeList', error);
            res.status(200).send({ count: 0, lisResponce1: [{}] });
        }
    }

    // change the status of recharge
    acceptBundleRecharge = async (req, res) => {
        this.#acceptBundleRecharge(req, res, 1)
    }

    acceptRechargeNoResponse = async (req, res) => {
        this.#acceptBundleRecharge(req, res, 0)
    }

    #acceptBundleRecharge = async (req, res, sendSms) => {
        try {
            // check body and query
            const errors = validationResult(req);
            if (!errors.isEmpty()) {
            ...
            // console.log('recharge/acceptBundleRecharge',JSON.stringify(req.body), JSON.stringify(req.query))
            var date = new Date();
            date.setHours(date.getHours() + 4, date.getMinutes() + 30);
            var isodate = date.toISOString();
    ...
    successToFailedBundleRecharge = async (req, res) => {
        try {
            // check body and query
            const errors = validationResult(req);
            if (!errors.isEmpty()) {
            ...
            console.log('recharge/successToFailedBundleRecharge', JSON.stringify(req.body), JSON.stringify(req.query))
            var date = new Date();
            date.setHours(date.getHours() + 4, date.getMinutes() + 30);
            var isodate = date.toISOString();
