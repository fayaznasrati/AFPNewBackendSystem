...
const {
  ...
  sendMessage,
  createWorker,
} = require("../common/rabbitmq.common");
...
class rechargeController {
    ...
    start().then((msg) => {
      ...
      if (process.env.START_RABBIT_MQ_WORKER == 1) {
        ...
        if (msg == "connection created") {
          ...
          createWorker(
            "refundFailedRechargeBalance",
            this.refundFailedRechargeBalance
          );
          createWorker(
            "rollbackCommissionFromAgent",
            this.rollbackCommissionFromAgent
          );
        }
      }
    });
  ...
  failedRechargeRollback = async () => {
    try {
      ...
      const lisResponce1 = await commonQueryCommon.getOperatorById(
        data.operator_uuid
      );
      if (lisResponce1 == 0)
        return { status: 400, message: "operator id not found" };
      ...
      // get system balance
      let balanceDe = await sqlQuery.searchQuery(
        this.tableName20,
        { id: data.mno_id, status: 1 },
        [
          "current_balance",
          "CAST(mno_uuid AS CHAR(16)) AS mno_uuid",
          "mno_name",
        ],
        "id",
        "desc",
      ...
      var objResponce = await sqlQuery.updateQuery(
        this.tableName4,
        { canTransfer: 0 },
        searchKeyValue
      );
      ...
      const lisResponce3 = await sqlQuery.searchQueryTran(
        this.tableName4,
        { user_uuid: data.user_uuid },
        ["ex_wallet", "min_wallet"],
        "userid",
        "ASC",
        1,
        0
      ...
      var objResponce = await sqlQuery.updateQuery(this.tableName20, keyValue, {
        id: data.mno_id,
      });
      ...
      var param = {
        ...
        closing_balance:
          Number(balanceDe[0].current_balance) + Number(data.amount), //db closing balance
  ...
  groupRecharge = async (req, res) => {
    try {
      ...
      const lisResponce4 = await sqlQueryReplica.searchQueryNoLimit(
        this.tableName7,
        { group_uuid: req.body.group_uuid, active: 1 },
        ["mobile", "group_id", "amount"],
        "name",
        "ASC"
      );
      ...
      if (lisResponce4.length == 0) {
        ...
        return res
          .status(400)
          .json({
            errors: [{ msg: "Group list dont have any contact number" }],
          });
      }
  ...
  bulkTopupRecharge = async (req, res) => {
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const safeMobile = normalized
          ? String(normalized).trim()
          : "0000000000"; // fallback value
  ...
  #checkStockTransferStatus = async () => {
    let strStockTransferStatus = await redisMaster.asyncGet(
      "STOCK_TRANSFER_STATUS"
    );
    if (strStockTransferStatus) {
      return JSON.parse(strStockTransferStatus);
    } else {
      let stockTransferStatus = await sqlQuery.searchQueryNoCon(
        this.tableName24,
        ["stock_transfer"],
        "stock_transfer",
        "ASC",
      ...
      redisMaster.post(
        "STOCK_TRANSFER_STATUS",
        JSON.stringify(stockTransferStatus)
      );
  ...
  singlerecharge = async (req, res) => {
    try {
      ...
      if (
        stockTransferStatus.length == 0 ||
        stockTransferStatus[0].stock_transfer == 0
      ) {
        responce = {
          status: 400,
          message: "Recharge is not allowed for a while.",
        };
      } else {
        responce = await this.processRecharge(data);
      }
      ...
      if (responce.status == 200)
        res.status(responce.status).send({ message: responce.message });
      else
        res
          .status(responce.status)
          .json({ errors: [{ msg: responce.message }] });
    } catch (error) {
      res.status(400).json({ errors: [{ msg: error.message }] });
    }
  };
  ...
  #getOperatorList = async (operator_uuid) => {
    ...
    if (strOperatorDetails) {
      ...
      if (lisResponce1)
        redisMaster.post(operatorUuid, JSON.stringify(lisResponce1));
      return lisResponce1;
    }
  };
  ...
  #getOperatorSystemList = async (operator_id) => {
    ...
    if (strOperatorSystemList) {
      ...
      let systemDetails = await sqlQuery.searchQueryNoLimit(
        this.tableName6,
        { operator_id: operator_id, active: 1 },
        ["mno_id", "queue_name", "status", "min_amount", "max_amount"],
        "operator_access_id",
        "DESC"
      );
  ...
  #getOperatorAccess = async (user_uuid, operatorId) => {
    ...
    if (strOperatorPermission) {
      ...
      lisResponce2 = await sqlQuery.searchQuery(
        this.tableName2,
        { user_uuid: user_uuid },
        [
          "oper1_status",
  ...
  #getUserChannel = async (user_uuid, channelType) => {
    ...
    if (strChannelPermission) {
      ...
      channelList = await sqlQuery.searchQueryNoLimit(
        this.tableName14,
        { user_uuid: user_uuid },
        ["threshold", "status", "channel"],
        "userid",
        "ASC",
        1,
        0
  ...
  processRecharge = async (data) => {
    try {
      ...
      // get system balance
      let systemDetails = await this.#getOperatorSystemList(
        lisResponce1[0].operator_id
      );
      if (systemDetails.length == 0)
        return { status: 400, message: "operator not active" };
      let mno_id,
        queue_name,
        minRechargeLimit,
        maxRechargeList,
      ...
      if (Number(minRechargeLimit) > Number(data.amount)) {
        return {
          status: 400,
          message: `Please enter amount more then or equal to ${minRechargeLimit}`,
        };
      }
      if (
        Number(maxRechargeList) > 0 &&
        Number(maxRechargeList) < Number(data.amount)
      )
        return {
          status: 400,
          message: `Please enter amount less then or equal to ${maxRechargeList}`,
        };

      // get recharge count
      let pendingCount = await redisMaster.incr(
        `PENDING_RECHARGE_${lisResponce1[0].operator_id}`
      );
      let allowedPendingCount = await redisMaster.asyncGet(
        `PENDING_ALLOWED_${lisResponce1[0].operator_id}`
      );
      ...
      if (Number(data.amount) > Number(balanceDe[0].current_balance)) {
        ...
        return {
          status: 400,
          message:
            "We have encountered an issue on wallet, Please contact our Customer Service",
        };
      }
      ...
      if (lisResponce2[0].comm_type == 0) {
        ...
        return {
          status: 400,
          message: "Commission not set feature not active",
        };
      }

      // check if recahnge is done in last 5 min
      var lastRechangeCount = await sqlQuery.searchQuery(
        this.tableName1,
        {
      ...
      if (lastRechangeCount[0]["COUNT(1)"] != 0) {
        ...
        return {
          status: 400,
          message: `Same recharge is already done within ${process.env.RECHARGE_TIME_LIMIT} min`,
        };
      }
      // update agent account so it cant do any transactions
      var searchKeyValue = {
      ...
      if (lisResponce3.length == 0) {
        ...
        return {
          status: 400,
          message: "user dont have enough balance to do recharge",
        };
      }
      ...
      let channelType = channelList.includes(data.channelType)
        ? data.channelType
        : "Web";
      ...
      let channelLimit = await this.#getUserChannel(
        data.user_uuid,
        channelType
      );
      ...
      if (channelLimit[0].status != 1) {
        ...
        return {
          status: 400,
          message: `Your ${data.channelType} channel is In-Active.`,
        };
      }

      if (
        Number(channelLimit[0].threshold) >
        Number(lisResponce3[0].ex_wallet) - Number(data.amount)
      ) {
        var lisresponce = await sqlQuery.specialCMD("rollback");
        var searchKeyValue = {
          user_uuid: data.user_uuid, //str user_uuid
        ...
        return {
          status: 400,
          message: `Your ${data.channelType} channel Threshold limit reached.`,
        };
      }

      if (lisResponce2[0].comm_type == "1") {
        let limitStatus = await sqlQuery.searchQueryTran(
          this.tableName13,
          { userid: data.userid },
          [
            "op" +
              lisResponce1[0].operator_id +
              "_wallet_active AS limit_state",
            "op" + lisResponce1[0].operator_id + "_wallet_limit as limit_value",
          ],
          "userid",
          "ASC",
          1,
        ...
        if (limitStatus[0].limit_state == 1) {
          ...
          if (Number(limitStatus[0].limit_value) - Number(data.amount) >= 0) {
            // update limit
            let updateLimit = await sqlQuery.updateQuery(
              this.tableName13,
              {
                deductBalance: {
            ...
            return {
              status: 400,
              message: "Recharge amount is more then Operator limit",
            };
          }
        }
      }
      ...
      var logData = {
        ...
        description: `Dear ${data.full_name}, Your Recharge to ${
          data.mobile
        } Amount: ${parseFloat(String(data.amount)).toFixed(
          2
        )} AFN is Accepted, Bal:${
          Number(lisResponce3[0].ex_wallet) - Number(data.amount)
        } AFN TX:${strUniqueNumber} Thank You for being Afghan Pay agent!`,
        userActivityType: 25,
      ...
      // console.log("data",data);
      let responceMessage = `Dear ${data.full_name}, Your Recharge to ${
        data.mobile
      } Amount: ${parseFloat(String(data.amount)).toFixed(
        2
      )} AFN is Accepted, Bal:${
  ...
  CompanyProcessRecharge = async (data) => {
    try {
      ...
      const lisResponce1 = await this.#getOperatorList(data.operator_uuid);
      if (lisResponce1 == 0)
        return { status: 403, message: "operator id not found" };
      ...
      if (systemDetails.length == 0)
        return { status: 403, message: "operator is not active" };
      let mno_id,
        queue_name,
        minRechargeLimit,
        maxRechargeList,
      ...
      if (lisResponce3.length == 0) {
        ...
        return {
          status: 402,
          message: "user do not have enough balance to do recharge",
        };
      }
      if (lisResponce3[0].ex_wallet - Number(data.amount) < 0) {
        var lisresponce = await sqlQuery.specialCMD("rollback");
      ...
      var logData = {
        ...
        userAppVersion: data.userAppVersion, //str
        userApplicationType: data.userApplicationType,
        description: `Dear valuable partner ${
          data.full_name
        }, Your Recharge to ${data.mobile} Amount: ${parseFloat(
          String(data.amount)
        ).toFixed(2)} AFN is Accepted, Bal:${
          Number(lisResponce3[0].ex_wallet) - Number(data.amount)
        } AFN TX:${strUniqueNumber} and Company TX:${
          data.company_transaction_id
        } Thank you for being an Afghan Pay partner!`,
        userActivityType: 25,
        oldValue: Number(lisResponce3[0].ex_wallet),
        newValue: Number(lisResponce3[0].ex_wallet) - Number(data.amount),
        regionId: data.region_id,
      ...
      var lisresponce = await sqlQuery.specialCMD("commit");
      // console.log("data",data);
      let responceMessage = `Dear valuable partner ${
        data.full_name
      }, Your Recharge to ${data.mobile} Amount: ${parseFloat(
        String(data.amount)
      ).toFixed(2)} AFN is Accepted, Bal:${
        Number(lisResponce3[0].ex_wallet) - Number(data.amount)
      } AFN TX:${strUniqueNumber}  Thank you for being an Afghan Pay partner!`;
  ...
  getPendingRechargeList = async (req, res) => {
    try {
      ...
      if (lisResponce1.length == 0)
        return res.status(204).send({ message: "no pending recharge found" });
  ...
  #acceptRecharge = async (req, res, sendSms) => {
    try {
      ...
      // update status to
      let updateStatusResponce = await sqlQuery.updateQuery(
        this.tableName1,
        { status: 4 },
        { trans_number: req.body.transNumber, status: 1 }
      );
      if (
        !(updateStatusResponce.affectedRows && updateStatusResponce.changedRows)
      )
        return res
          .status(400)
          .json({ errors: [{ msg: "Transaction Not Found" }] });
      ...
      var key = [
        ...
        "request_mobile_no",
        "source",
        "CAST(created_on as CHAR(16)) AS created_on",
      ];
      const lisResponce1 = await sqlQuery.searchQuery(
        this.tableName1,
        searchKeyValue,
        key,
        "userid",
      ...
      switch (lisResponce1[0].operator_id) {
        case "1":
          updateResponce = await this.updateSalamResponce(
            req.body.transNumber,
            req.body.rechargeRequest,
            req.body.rechargeResponce,
            1
          );
          break;
        case "2":
          updateResponce = await this.updateAwccResponce(
            req.body.transNumber,
            req.body.rechargeRequest,
            req.body.rechargeResponce,
            1
          ...
          break;
        case "3":
          updateResponce = await this.updateMtnResponce(
            req.body.transNumber,
            req.body.rechargeRequest,
            req.body.rechargeResponce,
            1
          ...
          break;
        case "4":
          updateResponce = await this.updateEtisalatResponce(
            req.body.transNumber,
            req.body.rechargeRequest,
            req.body.rechargeResponce,
            1
          ...
          break;
        case "5":
          updateResponce = await this.updateRoshanResponce(
            req.body.transNumber,
            req.body.rechargeRequest,
            req.body.rechargeResponce,
            1
      ...
      const lisResponce2 = await sqlQuery.searchQuery(
        this.tableName2,
        { userid: lisResponce1[0].userid, Active: 1 },
        ["comm_type", "region_id", "prefer_lang"],
        "userid",
        "ASC",
        1,
        0
      ...
      if (lisResponce2.length == 0) {
        ...
        return res
          .status(400)
          .json({ errors: [{ msg: "agent details not found" }] });
      }
      ...
      sendMessage(
        "rechargeSuccessAddSystemSummery",
        JSON.stringify(messageQueue),
        (err, msg) => {
          if (err) console.log(err);
        }
      ...
      if (lisResponce2[0].comm_type == 2) {
        distributeCommissionResponce = await this.distributeCommission(
          lisResponce1,
          req.body.transNumber,
          lisResponce2[0].region_id
        );
        ...
        if (distributeCommissionResponce.error) {
          ...
          return res
            .status(400)
            .json({ errors: [{ msg: distributeCommissionResponce.error }] });
        }
      }
      ...
      // update the status
      const lisResponce3 = await sqlQuery.updateQuery(
        this.tableName1,
        param,
        searchKeyValue
      );
      ...
      // get agent details
      let agentDetails = await sqlQuery.searchOrQuery(
        this.tableName2,
        { userid: lisResponce1[0].userid },
        [
          "userid",
      ...
      let smsDetails = {
        ...
        recieverMessage: `Dear ${agentDetails[0].full_name} ${
          agentDetails[0].username
        }, Your Recharge to ${
          lisResponce1[0].mobile_number
        } Amount: ${parseFloat(String(lisResponce1[0].amount)).toFixed(
          2
        )} AFN is Success, Bal: ${parseFloat(
          String(lisResponce1[0].closing_balance)
        ).toFixed(2)} AFN TX:${
          req.body.transNumber
        } Thank You for being Afghan Pay agent!`,
      };

      switch (String(lisResponce2[0].prefer_lang)) {
        case "2": // Pashto
          smsDetails.recieverMessage = `ښاغلی ${
            agentDetails[0].full_name
          }! شمېرې ${lisResponce1[0].mobile_number} ته  ${parseFloat(
            String(lisResponce1[0].amount)
          ).toFixed(
            2
          )} افغانۍ په بریالیتوب سره ولېږل شوې، ستاسو پاتی کریدیت ${parseFloat(
            String(lisResponce1[0].closing_balance)
          ).toFixed(2)} افغانی دی. مننه، افغان پی.`;
          break;
        case "3": // Dari
          smsDetails.recieverMessage = `محترم ${
            agentDetails[0].full_name
          }! به شماره ${lisResponce1[0].mobile_number} مبلغ ${parseFloat(
            String(lisResponce1[0].amount)
          ).toFixed(2)} افغانی موفقانه ارسال شد، کریدیت فعلی شما ${parseFloat(
            String(lisResponce1[0].closing_balance)
          ).toFixed(2)} افغانی است، افغان پی.`;
          break;
        case "1": // english
        default:
      ...
      smsFunction
        .agentSms(smsDetails, lisResponce1[0].request_mobile_no)
        .then((smsFunResponce) => {
      ...
      smsDetails = {
        ...
        recieverMessage: `Your Mobile has been successfully Recharged with ${parseFloat(
          String(lisResponce1[0].amount)
        ).toFixed(2)} AFN by ${agentDetails[0].full_name}, TXN:${
          req.body.transNumber
        } Thank You (Afghan Pay)!`,
      };
      switch (String(lisResponce2[0].prefer_lang)) {
        case "2": // Pashto
          smsDetails.recieverMessage = `${parseFloat(
            String(lisResponce1[0].amount)
          ).toFixed(2)} افغانۍ کریډیټ ستاسو شمېرې ته د ${
            agentDetails[0].full_name
          } لخوا په بریالۍ توګه زیاته شوه. مننه، افغان پی.`;
          break;
        case "3": // Dari
          smsDetails.recieverMessage = `مبلغ ${parseFloat(
            String(lisResponce1[0].amount)
          ).toFixed(2)} افغانی کریدت به شماره شما توسط ${
            agentDetails[0].full_name
          } موفقانه اضافه گردید، تشکر، افغان پی.`;
          break;
        case "1": // english
        default:
          smsDetails.recieverMessage = `Your Mobile has been successfully Recharged with ${parseFloat(
            String(lisResponce1[0].amount)
          ).toFixed(2)} AFN by ${agentDetails[0].full_name}, TXN:${
            req.body.transNumber
          } Thank You (Afghan Pay)!`;
      ...
      var logData = {
        ...
        userApplicationType:
          lisResponce1[0].source == "Web"
            ? 1
            : lisResponce1[0].source == "Mobile"
            ? 2
            : lisResponce1[0].source == "USSD"
            ? 3
            : 4,
        ...
        userActivityType: 25,
        oldValue: `Recharge Accepted ${parseFloat(
          String(lisResponce1[0].amount)
        ).toFixed(2)}`,
        newValue: `Closing Balance ${parseFloat(
          String(lisResponce1[0].closing_balance)
        ).toFixed(2)}`,
  ...
  resendConformationSms = async (req, res) => {
    try {
      ...
      // get agent details
      let agentDetails = await sqlQueryReplica.searchOrQuery(
        this.tableName2,
        { userid: lisResponce1[0].userid },
        [
          "userid",
  ...
  addRechargeAmount = async (userId, amount, operatorId) => {
    ...
    let getCommissionPercentage = await sqlQuery.updateQuery(
      this.tableName22,
      key,
      { userid: userId, "CAST(date AS DATE)": isodate }
    );
    ...
    let getParentId = await sqlQuery.searchQuery(
      this.tableName2,
      { userid: userId },
      ["parent_id"],
      "userid",
    ...
    if (getParentId[0].parent_id != 1) {
      this.addRechargeAmountUpline(
        getParentId[0].parent_id,
        amount,
        operatorId
      );
    }
  ...
  distributeCommission = async (lisResponce1, strUniqueNumber, regionId) => {
    try {
      ...
      let userid = lisResponce1[0].userid,
        commission = 0,
        givenCommissionPer = 0,
        commissionAmount = 0,
        amount = Number(lisResponce1[0].amount),
        operatorId = lisResponce1[0].operator_id;
      let searchKeyValue,
        key,
        lisResponce3,
      ...
      while (true) {
        ...
        key = [
          "CAST(user_uuid as CHAR(16)) AS user_uuid",
          "parent_id",
          "op" + operatorId + "_comm AS commissionPer",
        ];
        lisResponce3 = await sqlQuery.searchQuery(
          this.tableName8,
          searchKeyValue,
          key,
          "userid",
        ...
        givenCommissionPer =
          Number(lisResponce3[0].commissionPer) - commission > 0
            ? Number(lisResponce3[0].commissionPer) - commission
            : 0;
        commissionAmount = (givenCommissionPer * amount) / 100;
        commission = lisResponce3[0].commissionPer;
        agnetCommissionPer =
          agnetCommissionPer == 0
            ? Number(lisResponce3[0].commissionPer)
            : agnetCommissionPer;
        agentCommissionAmount =
          agentCommissionAmount == 0 ? commissionAmount : agentCommissionAmount;
        ...
        let commissionResponce = await sqlQuery.createQuery(
          this.tableName9,
          commissionDetails
        );
      ...
      return {
        ...
        agentCommissionAmount: rechargeAmount,
        agnetCommissionPer: rechargeCommission,
      };
    } catch (error) {
  ...
  giveCommissionToAgent = async (userDetailsString) => {
    try {
      ...
      // fire sql update query to change the can transfer status to 0 only when the can transfer is 1
      let objResponce = await sqlQuery.updateQuery(
        this.tableName4,
        { canTransfer: 0 },
        searchKeyValue
      );
      ...
      if (!(affectedRows && changedRows)) {
        ...
        var lisresponce = await sqlQuery.specialCMD("transaction");
        // get wallet balance
        let lisResponce4 = await sqlQuery.searchQueryTran(
          this.tableName4,
          { user_uuid: userDetails.user_uuid },
          ["ex_wallet", "comm_wallet"],
          "userid",
          "ASC",
          1,
          0
        ...
        // fire sql update query to change the can transfer status to 0 only when the can transfer is 1
        objResponce = await sqlQuery.updateQuery(
          this.tableName4,
          param,
          searchKeyValue
        );
        ...
        param = {
          ...
          narration: "Top-Up commission",
          balance_amount:
            Number(lisResponce4[0].ex_wallet) +
            Number(userDetails.commissionAmount), //db balance amount
          trans_for: "Top-Up commission",
        };
        //fire sql query
        objResponce = await sqlQuery.createQuery(this.tableName5, param);
  ...
  updateSalamResponce = async (id, request, responce, status) => {
    try {
      ...
      let updateResponce = await sqlQuery.updateQuery(
        this.tableName15,
        keyValue,
        { recharge_id: id }
      );
  ...
  rejectRecharge = async (req, res) => {
    try {
      ...
      let param = {
        ...
        operator_balance: req.body.mnoBalance || 0,
        closing_balance:
          Number(lisResponce1[0].closing_balance) +
          Number(lisResponce1[0].amount),
        concat: {
          key: "os_details",
          value: " @@ " + req.body.rechargeResponce,
        },
      ...
      // update the status
      const lisResponce2 = await sqlQuery.updateQuery(
        this.tableName1,
        param,
        searchKeyValue
      );
      ...
      let balanceDe = await sqlQuery.searchQuery(
        this.tableName20,
        { id: lisResponce1[0].api_type, status: 1 },
        [
          "current_balance",
          "CAST(mno_uuid AS CHAR(16)) AS mno_uuid",
          "mno_name",
      ...
      let objResponce = await sqlQuery.updateQuery(this.tableName20, keyValue, {
        id: lisResponce1[0].api_type,
      });
      var { affectedRows, changedRows, info } = objResponce;
      if (!(affectedRows && changedRows)) {
        var lisresponce = await sqlQuery.specialCMD("rollback");
      ...
      param = {
        ...
        opening_balance: balanceDe[0].current_balance, //db opening balance
        closing_balance:
          Number(balanceDe[0].current_balance) + Number(lisResponce1[0].amount), //db closing balance
      ...
      // get agent name
      const agentName = await sqlQuery.searchQuery(
        this.tableName2,
        { userid: lisResponce1[0].userid },
        [
          "userid",
      ...
      let smsDetails = {
        agentId: lisResponce1[0].userid,
        recieverMessage: `Dear ${agentName[0].full_name} ${
          agentName[0].username
        }, Your Recharge to ${
          lisResponce1[0].mobile_number
        } Amount: ${parseFloat(String(lisResponce1[0].amount)).toFixed(
          2
        )} AFN is Failed, Bal: ${parseFloat(
          String(
            Number(lisResponce1[0].closing_balance) +
              Number(lisResponce1[0].amount)
          )
      ...
      switch (String(agentName[0].prefer_lang)) {
        case "2": // Pashto
          smsDetails.recieverMessage = `ښاغلی ${
            ...
            2
          )} افغانیو په اندازه د کریډیټ لیږل ناکام شول، ستاسو اوسنی کریډیټ ${parseFloat(
            String(
              Number(lisResponce1[0].closing_balance) +
                Number(lisResponce1[0].amount)
            )
        ...
        case "3": // Dari
          smsDetails.recieverMessage = `محترم ${
            agentName[0].full_name
          }! ارسال کریدت به شماره ${
            lisResponce1[0].mobile_number
          } مبلغ ${parseFloat(String(lisResponce1[0].amount)).toFixed(
            2
          )} افغانی ناکام گردید، کریدیت فعلی شما ${parseFloat(
            String(
              Number(lisResponce1[0].closing_balance) +
                Number(lisResponce1[0].amount)
            )
          ).toFixed(2)} افغانی است. تشکر،افغان پی.`;
          break;
        case "1": // english
        default:
      ...
      var logData = {
        ...
        userActivityType: 25,
        oldValue: `Recharge Rejected ${parseFloat(
          String(lisResponce1[0].amount)
        ).toFixed(2)}`,
        newValue: `Closing Balance ${parseFloat(
          String(
  ...
  getMnoResponse = async (req, res) => {
    try {
      ...
      var key = [
        ...
        "CAST(created_on AS CHAR(20)) as recharge_date",
        "CAST(modified_on AS CHAR(20)) as update_on",
        "rollback_status as rollback",
        "CAST(rollback_confirm_on AS CHAR(20)) as rollback_date",
        "os_details as mno_response",
      ];
  ...
  successToFailedRecharge = async (req, res) => {
    try {
      ...
      console.log(
        "recharge/successToFailedRecharge",
      ...
      sendMessage(
        "rechargeFailedDeductUserSummery",
        JSON.stringify(messageQueue),
        (err, msg) => {
          if (err) console.log(err);
        }
      ...
      sendMessage(
        "rechargeFailedDeductSystemSummery",
        JSON.stringify(messageQueue),
        (err, msg) => {
          if (err) console.log(err);
        }
      ...
      if (agentName[0].comm_type == 2) {
        this.rollbackCommission(
          req.body.transNumber,
          lisResponce1[0].operator_id,
          lisResponce1[0].created_on
        );
      }
  ...
  rollbackCommission = async (strUniqueNumber, operatorId, dateTime) => {
    try {
      ...
      let rechargeList = await rechargeModel.getCommissionRollbackList(
        strUniqueNumber,
        operatorId
      );
  ...
  refundFailedRechargeBalance = async (message) => {
    try {
      ...
      let sqlResponse = await sqlQuery.searchQuery(
        this.tableName5,
        searchQuery,
        ["count(1)"],
        "trans_number",
      ...
      if (sqlResponse[0]["count(1)"] == 1) {
        ...
        if (!(affectedRows && changedRows)) {
          ...
          if (lisResponce3.length == 0) {
            ...
            return res
              .status(400)
              .json({ errors: [{ msg: "User wallet detailsn not found" }] });
          }
          ...
          if (!(affectedRows && changedRows)) {
            ...
            return res
              .status(400)
              .json({ errors: [{ msg: "Amount not reversed" }] });
          }
          ...
          param = {
            ...
            narration: userDetails.narration
              ? userDetails.narration
              : `Top-Up refund for ${userDetails.mobileNumber}`,
            balance_amount:
              Number(lisResponce3[0].ex_wallet) + Number(userDetails.amount), //db balance amount
  ...
  failedCountCheck = async (mnoId) => {
    try {
      ...
      if (failedRechargeCount[mnoId] == 1) {
        ...
        let mobileNumberList = await rechargeModel.failedRechargeNumberList(
          failedRechargeCount[mnoId]
        );
  ...
  topUpreports = async (req, res) => {
    try {
      ...
      if (req.query.operator_uuid) {
        const lisResponce1 = await commonQueryCommon.getOperatorById(
          req.query.operator_uuid
        );
        if (lisResponce1 == 0)
          return res
            .status(400)
            .json({ errors: [{ msg: "operator id not found" }] });
      ...
      if (Object.keys(searchKeyValue).length == 0)
        return res
          .status(400)
          .json({ errors: [{ msg: "Im proper search paremeters" }] });
    ...
    try {
      ...
      if (req.query.filterType && req.query.filterValue) {
        if (req.query.filterType == 1 || req.query.filterType == 2) {
          ...
          if (req.query.filterType == 1 || req.query.filterType == "1")
            searchKeyValue.username = req.query.filterValue;
          if (req.query.filterType == 2 || req.query.filterType == "2")
            searchKeyValue.mobile = req.query.filterValue;

          if (Object.keys(searchKeyValue).length == 2)
            return res
              .status(400)
              .json({ errors: [{ msg: "Impoper search parementer" }] });

          // search child
          var lisResponce1 = await sqlQueryReplica.searchQuery(
            this.tableName2,
            searchKeyValue,
            ["userid", "child_id"],
            "userid",
          ...
          searchkeyval1.child_ids =
            lisResponce1[0].child_id != ""
      ...
      if (Object.keys(searchkeyval1).length == 0)
        return res
          .status(400)
          .json({ errors: [{ msg: "Improper search parameter" }] });
      ...
      const lisTotalRecords = await rechargeModel.downlineTopUpReportCount(
        searchkeyval1
      );
      ...
      const lisResponce2 = await rechargeModel.downlineTopUpReport(
        searchkeyval1,
        limit,
        offset
      );
    ...
    try {
      ...
      if (req.query.group_uuid) {
        ...
        if (lisResponce1.length == 0)
          return res
            .status(400)
            .json({ errors: [{ msg: "group id not found" }] });
        searchKeyValue.group_topup_id = lisResponce1[0].group_id;
      }
      if (req.query.mobile) searchKeyValue.mobile_number = req.query.mobile;
      if (req.query.operator_uuid) {
        const lisResponce3 = await commonQueryCommon.getOperatorById(
          req.query.operator_uuid
        );
        if (lisResponce3 == 0)
          return res
      ...
      const lisTotalRecords = await sqlQueryReplica.searchQueryNoLimitTimeout(
        ...
        [
          ...
          "SUM(comm_amt) AS totalCommissionAmount",
        ],
        "id",
        "DESC"
      );
  ...
  telcoWiseTopUpReport = async (req, res) => {
    try {
      ...
      const lisResponce1 = await sqlQueryReplica.searchQueryNoLimitTimeout(
        this.tableName1,
        searchKeyValue,
        key,
        "operator_id",
      ...
      if (lisResponce1.length == 0)
        return res.status(204).send({ message: "no search result found" });
      ...
      for (i = 0; i < lisResponce1.length; i++) {
        if (operatorIds.includes(lisResponce1[i].operator_id)) {
          ...
          finalResult[operatorIndex].txnsAmount +=
            lisResponce1[i].status == 2 ? lisResponce1[i].amount : 0;
          finalResult[operatorIndex].successTxns +=
            lisResponce1[i].status == 2 ? 1 : 0;
          finalResult[operatorIndex].pendingsTxns +=
            lisResponce1[i].status == 1 ? 1 : 0;
          finalResult[operatorIndex].failedTxns +=
            lisResponce1[i].status == 3 ? 1 : 0;
        } else {
  ...
  agentTopupReport = async (req, res) => {
    try {
      ...
      // let sum and count
      const lisTotalRecords = await rechargeModel.agentTopupSumCountReport(
        searchKeyValue
      );
      ...
      const lisResponce1 = await rechargeModel.agentTopupReport(
        searchKeyValue,
        limit,
        offset
      );
  ...
  downloadAgentTopupReport = async (req, res) => {
    try {
      ...
      if (req.query.userId) {
        const userId = req.query.userId.startsWith("AFP-")
          ? req.query.userId
          : `AFP-${req.query.userId}`;
      ...
      if (req.query.userName) {
        if (!isNaN(req.query.userName)) {
          searchKeyValue.request_mobile_no = [
            req.query.userName,
            "0" + req.query.userName,
          ];
      ...
      if (req.query.operator_uuid) {
        const operator = await commonQueryCommon.getOperatorById(
          req.query.operator_uuid
        );
      ...
      while (true) {
        ...
        const chunk = await rechargeModel.downloadAgentTopupReport(
          searchKeyValue,
          perPage,
          offset
        );
        ...
        if (isFirstBatch) {
          sheet.columns = Object.keys(chunk[0]).map((key) => ({
            header: key,
            key,
          }));
          isFirstBatch = false;
  ...
  downloadAgentTopupReportpdf = async (req, res) => {
    try {
      ...
      while (true) {
        const offset = (page - 1) * perPage;
        const chunk = await rechargeModel.downloadAgentTopupReportpdf(
          searchKeyValue,
          perPage,
          offset
        );
  ...
  topUpSummeryReport = async (req, res) => {
    try {
      ...
      if (req.query.parent_uuid) {
        let parentDetails = await sqlQueryReplica.searchQuery(
          this.tableName2,
          {
            // region_ids : req.body.user_detials.region_list.join(','),
      ...
      let lisTotalRecords = await sqlQueryReplica.searchQueryNoLimitTimeout(
        this.tableName2,
        searchKeyValue,
        ["COUNT(1) AS count"],
        "userid",
      ...
      let agentList = await sqlQueryReplica.searchQueryTimeout(
        this.tableName2,
        searchKeyValue,
        [
          "username",
          ...
          "province_Name",
          "region_name",
          "IF(usertype_id = 1,'Master Distributor',IF(usertype_id = 2,'Distributor',IF(usertype_id = 3,'Reseller','Retailer'))) as agentType",
        ],
        "userid",
        "ASC",
        limit,
      ...
      for (let i = 0; i < agentList.length; i++) {
        ...
        let rechargeDetails = await rechargeModel.topUpSummeryReport(
          searchKeyValue,
          child_id
        );
  ...
  downloadTopUpSummeryReport = async (req, res) => {
    try {
      ...
      if (req.query.parent_uuid) {
        const parentDetails = await sqlQueryReplica.searchQuery(
          this.tableName2,
          { user_uuid: req.query.parent_uuid },
          ["userid"],
          "userid",
      ...
      for (const agent of agentList) {
        ...
        const rechargeDetails = await rechargeModel.topUpSummeryReport(
          rechargeSearch,
          allChildIds
        );
        rows.push({
  ...
  agentDownlineTopUpReport = async (req, res) => {
    try {
      ...
      if (req.query.parent_uuid) {
        ...
        if (lisResponce1.length == 0)
          return res.status(400).json({ errors: "parent id ont found" });
      ...
      if (Object.keys(searchKeyValue).length == 0)
        return res
          .status(400)
          .json({ errors: [{ msg: "Improper search key parem" }] });

      const lisTotalRecords = await rechargeModel.agentDownlineTopUpReportCount(
        searchKeyValue
      );
      ...
      const lisResponce2 = await rechargeModel.agentDownlineTopUpReport(
        searchKeyValue,
        limit,
        offset
      );
    ...
    try {
      ...
      if (req.query.parent_uuid) {
        ...
        if (!parentResult.length)
          return res.status(400).json({ errors: "parent id not found" });
      ...
      while (true) {
        const offset = (page - 1) * perPage;
        const chunk = await rechargeModel.agentDownlineTopUpReport(
          searchKeyValue,
          perPage,
          offset
        );
  ...
  agentTelcoTopUpreport = async (req, res) => {
    try {
      ...
      let searchResult = await rechargeModel.agentTelcoTopUpreport(
        searchKeyValue
      );
      ...
      let finalResult = searchResult.map((result) => {
        ...
        totalSum.successTopupAmount =
          totalSum.successTopupAmount + other.successTopupAmount;
        totalSum.successToupUpCount =
          totalSum.successToupUpCount + other.successToupUpCount;
  ...
  agentCommissionReport = async (req, res) => {
    try {
      ...
      if (listOperator.length == 0)
        return res.status(400).json({ errors: "Operator id not found" });
      ...
      if (req.query.userName) searchKeyValue.full_name = req.query.userName;
      if (req.query.agent_type_uuid) {
        const lisResponce1 = await commonQueryCommon.getAgentTypeId(
          req.query.agent_type_uuid
        );
        if (lisResponce1.length == 0)
          return res.status(400).json({ errors: "Agent type id not found" });
      ...
      const lisTotalRecords = await rechargeModel.agentCommissionReportCount(
        searchKeyValue
      );
      ...
      // search paremters
      const lisResponce2 = await rechargeModel.agentCommissionReport(
        searchKeyValue,
        limit,
        offset
      );
  ...
  downloadAgentCommissionReport = async (req, res) => {
    try {
      ...
      if (listAgentType.length === 0)
        return res.status(400).json({ errors: "Agent type list not found" });
      ...
      if (req.query.agent_type_uuid) {
        const agentTypeRes = await commonQueryCommon.getAgentTypeId(
          req.query.agent_type_uuid
        );
        if (!agentTypeRes || agentTypeRes.length === 0)
          return res.status(400).json({ errors: "Agent type not found" });
      ...
      if (Object.keys(searchKeyValue).length === 0)
        return res
          .status(400)
          .json({ errors: [{ msg: "Improper search parameters" }] });
      ...
      const totalPages =
        totalRecords % pageLimit === 0
          ? totalRecords / pageLimit
          : Math.floor(totalRecords / pageLimit) + 1;

      const offset =
        req.query.pageNumber > 0 ? (req.query.pageNumber - 1) * pageLimit : 0;
      const limit = req.query.pageNumber > 0 ? pageLimit : totalRecords;

      const resultData = await rechargeModel.agentCommissionReport(
        searchKeyValue,
        limit,
        offset
      );
  ...
  adminCommissionReport = async (req, res) => {
    try {
      ...
      const lisTotalRecords = await rechargeModel.adminCommissionReportCount(
        searchKeyValue,
        req.body.user_detials.userid
      );
      ...
      const lisresponce2 = await rechargeModel.adminCommissionReport(
        searchKeyValue,
        req.body.user_detials.userid,
        limit,
        offset
  ...
  adminDownloadCommissionReport = async (req, res) => {
    try {
      ...
      if (req.query.agent_type_uuid) {
        const agentType = await commonQueryCommon.getAgentTypeId(
          req.query.agent_type_uuid
        );
      ...
      const results = await rechargeModel.adminCommissionReport(
        searchKeyValue,
        userId,
        limit,
        offset
  ...
  commissionReport = async (req, res) => {
    try {
      ...
      if (req.query.topUpMobile)
        searchKeyValue.mobile_number = req.query.topUpMobile;
      if (
        (req.query.startDate && !req.query.endDate) ||
        (req.query.endDate && !req.query.startDate)
      )
      ...
      key = [
        "operator_name AS operatorName",
        "mobile_number AS topUpMobile",
        "amount AS topUpAmount",
        "closing_balance",
        "comm_amt",
        "CAST(created_on AS CHAR(20)) AS date",
      ];
  ...
  topRankingReport = async (req, res) => {
    try {
      ...
      let lisTotalRecords = await rechargeModel.topRankingReportCount(
        searchKeyValue
      );
      ...
      let finalResult = await rechargeModel.topRankingReport(
        searchKeyValue,
        limit,
        offset
      );
  ...
  downloadTopRankingReport = async (req, res) => {
    try {
      ...
      if (req.query.operator_uuid) {
        const operatorInfo = await commonQueryCommon.getOperatorById(
          req.query.operator_uuid
        );
      ...
      const totalRecordsList = await rechargeModel.topRankingReportCount(
        searchKeyValue
      );
      const intTotlaRecords = totalRecordsList.length;
      const pageLimit = Number(process.env.PER_PAGE_COUNT);
      const intPageCount =
        intTotlaRecords % pageLimit === 0
  ...
  groupTopUpReportByGroupId = async (req, res) => {
    try {
      ...
      // search goup id
      const groupId = await sqlQueryReplica.searchQueryTimeout(
        this.tableName10,
        { group_uuid: req.query.group_uuid, active: 1 },
        ["group_id"],
        "group_id",
        "ASC",
        1,
      ...
      var key = [
        "trans_number AS txnId",
        "operator_name AS operatorName",
        "mobile_number AS mobileNumber",
        "amount AS rechargeAmount",
        "deduct_amt AS deductedAmount",
        "operator_balance AS apiBalance",
        "CAST(created_on AS CHAR(20)) AS txnDate",
        "ap_transid AS operTxnId",
        "status AS txnStatus",
      ...
      const lisTotalRecords = await sqlQueryReplica.searchQueryNoLimitTimeout(
        ...
        [
          ...
          "SUM(deduct_amt) AS totalDeductedAmount",
        ],
        "id",
        "DESC"
      );
      ...
      const rechargeList = await sqlQueryReplica.searchQueryTimeout(
        this.tableName1,
        searchKeyValue,
        key,
        "id",
      ...
      if (rechargeList.length == 0)
        return res.status(204).send({ message: "no data to show" });
  ...
  responceUpdateRoshan = async (req, res) => {
    try {
      ...
      let updateResponce = await sqlQuery.updateQuery(
        this.tableName19,
        keyValue,
        { recharge_id: req.body.rechargeId }
      );
