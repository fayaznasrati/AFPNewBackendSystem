...
class AdminController {
    ...
    createUser = async(req, res, next) => {
        try {
            ...
            console.log('[Admin/createUser]', JSON.stringify(req.body), JSON.stringify(req.query));
    ...
    userLogin = async(req, res, next) => {
        try {
            ...
            console.log('[Admin/userLogin]', JSON.stringify(req.body), JSON.stringify(req.query));

            var userid = req.body.username;

            const userExist = await UserModel.matchPassword({ username: req.body.username, password: req.body.password });
            if (userExist == 1) {
            ...
            if (userExist == 2) {
                ...
                if( allowedAttempt-attempt <= 0 ){
                    ...
                    const updateResponce = await sqlQuery.updateQuery(this.tableName1,{status : 0},{username : req.body.username})
                }

                return res.status(400).json({ errors: [ {msg : `Wrong password,  ${allowedAttempt-attempt > 0 ? allowedAttempt-attempt : 0} left.`}] });
            } 
            if (userExist == 3) {
                return res.status(400).json({ errors: [ {msg : 'Account is In-Active'}] });
            }
            else {
                // console.log(process.env.SESSION_TIME)
                const secretKey = process.env.SECRET_JWT || "";
                const token = jwt.sign({ user_id: req.body.username, userType: userExist.usertype }, secretKey, {
                    ...
                    //get the last login date and ip address
                        var userLastLoginDetails = await sqlQueryReplica.searchQuery(this.tableName1, {username : req.body.username}, ["admin_userid","last_login_datetime","last_login_ip","usertype_id"], "admin_userid", "ASC", 1, 0)
                    ...
                    // update the details of last login 
                        var updateTheDetails = await sqlQuery.updateQuery(this.tableName1 , { last_login_datetime : isodate, last_login_ip : req.body.userIpAddress ? req.body.userIpAddress : 0 }, {username : req.body.username})
                    ...
                    // once login is allowed add the data in log table
                        ...
                        var strLog = intResult == 1 ? 'Admin login log added successfully' : intResult == 2 ? 'Admin login log error' : 'end point not found'
                    ...
                    // add log in another table
                        ...
                        var strLog = intResult1 == 1 ? 'Admin login log added successfully' : intResult1 == 2 ? 'Admin login log error' : 'end point not found'
                        // console.log('Server Log : '+strLog)
                    ...
                    if(userLastLoginDetails[0].usertype_id){
                        listModulePermission = await sqlQueryReplica.searchQuery(this.tableName8,{department_id : userLastLoginDetails[0].usertype_id},["sub_admin_module_name AS moduleName","perm_view AS permView"],"sub_admin_per_id","ASC",10,0)
    ...
    getUserByuserName = async(req, res, next) => {
        try {
            ...
            var key = ["username", "usertype", "full_name", "gender", "emailid", "mobile", "CAST(operator_uuid AS CHAR(16)) operator_uuid","operator_name","city_name", "Address"]
            var orderby = "full_name"
            var ordertype = "ASC"

            // fire sql query to get user id
            const lisResult = await sqlQueryReplica.searchQuery(this.tableName1, searchKeyValue, key, orderby, ordertype, 1, 0)
    ...
    updateUserDetials = async(req, res) => {
        try {
            // verify body and query
                ...
                console.log('[Admin/updateUserDetails]', JSON.stringify(req.body), JSON.stringify(req.query));
            // search user details
                var searchKeyValue = {
                ...
                var key = ["full_name","emailid","city_name","Address","gender","mobile"]
                var orderby = "full_name"
                var ordertype = "ASC"
                ...
                oldValue += req.body.name != lisResponce[0].full_name ? lisResponce[0].full_name + " ": ''
                oldValue += req.body.email != lisResponce[0].emailid ? lisResponce[0].emailid + " ": ''
                oldValue += req.body.city != lisResponce[0].city_name ? lisResponce[0].city_name + " ": ''
                oldValue += req.body.address != lisResponce[0].Address ? lisResponce[0].Address + " ": ''
                oldValue += (req.body.gender && (req.body.gender != lisResponce[0].gender)) ? lisResponce[0].gender + " " : ''

                newValue += req.body.name != lisResponce[0].full_name ? req.body.name + " ": ''
                newValue += req.body.email != lisResponce[0].emailid ? req.body.email + " ": ''
                newValue += req.body.city != lisResponce[0].city_name ? req.body.city + " ": ''
                newValue += req.body.address != lisResponce[0].Address ? req.body.address + " ": ''
                newValue += (req.body.gender && (req.body.gender != lisResponce[0].gender)) ? req.body.gender + " " : ''
            ...
            if(newValue.length > 0){
                // api call variable
                    var data = { 
                        ...
                        user_uuid : "0000000000000000",
                        username : req.query.username ,
                        full_name : req.body.name != lisResponce[0].full_name ? req.body.name : lisResponce[0].full_name ,
                ...
                return res.status(400).json({ errors: [ {msg : "user details are same"}] });
            }
            ...
            // update user details
                ...
                if(req.body.gender !== null && req.body.gender !== undefined) param.gender = req.body.gender
            ...
            // check if the result is there and responce accordingly
                ...
                const message = !affectedRows ? 'user not found' :
                    affectedRows && changedRows ? ' user details updated sucessfully' : ' user details are same';
    ...
    changePassword = async(req, res) => {
        try {            
            ...
            if (!errors.isEmpty()) {
                ...
                console.log('[Admin/changePassword]', JSON.stringify(req.body), JSON.stringify(req.query));
                var searchKeyValue = {
                    admin_userid: req.body.user_detials.id,
                    status : 1,
                    active : 1
                }
                var key = ["encryption_key", "password","tpin",'username','full_name',"mobile"]
                ...
                const hashOldPassword = this.getHashPassword(lisResponse1[0].encryption_key, lisResponse1[0].password)
                const storedpassword = lisResponse1[0]["tpin"];
                var strDecriptPin = this.getHashPassword(lisResponse1[0].encryption_key, storedpassword);
                strDecriptPin = strDecriptPin ? strDecriptPin : "000000"
                const newEncryptionKey = this.setEncryptionKey()
                const newPassword = this.setHashPassword(newEncryptionKey, req.body.password);
                const strEncryptedPin = this.setHashPassword(newEncryptionKey, strDecriptPin);
                ...
                if (hashOldPassword === req.body.oldPassword) {
                    ...
                    // api call variable
                        var data = { 
                            ...
                            oldValue : lisResponse1[0].password + " " + lisResponse1[0].encryption_key,
                            newValue : newPassword + " " + newEncryptionKey,
                            regionId : 0
                        }

                    // make api call
                        let intResult = await httpRequestMakerCommon.httpPost("activity-log",data)
                        var strLog = intResult == 1 ? 'Admin change password log added successfully' : intResult == 2 ? 'Admin chain password log error' : 'end point not found'
                            // console.log('Server Log : '+strLog)
                ...
                return res.status(400).json({ errors: [ {msg : 'old password dont match'}] });
            }
        } catch (error) {
            console.log(error);
            return res.status(400).json({ errors: [ {msg : error.message}] });
    ...
    requestForOtp = async ( req, res) => {
        try {
            ...
            const lisResponse2 = await sqlQueryReplica.searchQuery(this.tableName10, searchKeyValue, ["COUNT(1)"], 'operator_id', 'asc', 1, 0)
            ...
            var key = ['CAST(operator_uuid AS CHAR(16)) AS operator_uuid','admin_userid']
            var orderby = "mobile"
            var ordertype = "ASC"

            // fire sql query to get str country_uuid, str country_name
            const boolResponce = await sqlQuery.searchQuery(this.tableName1, searchKeyValue, key, orderby, ordertype, 1, 0)
            ...
            var key = ['mobile','CAST(operator_uuid AS CHAR(16)) AS operator_uuid','operator_name']
            var orderby = "mobile"
            var ordertype = "ASC"
    ...
    getOtp = async (req,res) => {
        try{
            ...
            console.log('[Admin/getOtp]', JSON.stringify(req.body), JSON.stringify(req.query));
            var searchKeyValue = {
    ...
    verifyAndUpdateNumber = async(req, res) => {
        try{
            ...
            // check if the result is there and responce accordingly
                const { affectedRows, changedRows, info } = objResult;
                const message = !affectedRows ? "Wrong OTP" :
                    affectedRows && changedRows ? 'Mobile number updated successfully' : 'Mobile number allready updated';
            ...
            // on update add into log table
                if(affectedRows && changedRows){
                    // get user old mobile number and operator name 
                        ...
                        var key = ['mobile','CAST(operator_uuid AS CHAR(16)) AS operator_uuid','operator_name','full_name','username']
                        var orderby = "mobile"
                        var ordertype = "ASC"
                    ...
                    // amke the api call
                        ...
                        // console.log(result)
                        var strLog = intResult == 1 ? 'Admin change mobile number log added successfully' : intResult == 2 ? 'Admin chain mobile number log error' : 'end point not found'
                        // console.log('Server Log : '+strLog)
                    ...
                    // make api call
                        const intResult1 = await httpRequestMakerCommon.httpPost("activity-log",data)
                        var strLog = intResult1 == 1 ? 'Admin change password log added successfully' : intResult1 == 2 ? 'Admin chain password log error' : 'end point not found'
                            // console.log('Server Log : '+strLog)
                }  
    ...
    getSecurityPin = async(req, res)=>{
        try{
            ...
            console.log('[Admin/getSecurityPin]', JSON.stringify(req.body), JSON.stringify(req.query));
            ...
            if(!lisResult[0].tpin) return res.status(200).send({ message: 'Pin not set'})
    ...
    verifySecurityPin = async ( req,res ) => {
        try{
            ...
            console.log('[Admin/verifySecurityPin]', JSON.stringify(req.body), JSON.stringify(req.query));
            var searchKeyValue = {
                username : req.body.user_detials.username,
                status : 1,
                active : 1
            ...
            const lisResponce1 = await sqlQuery.searchQueryNoLimit(this.tableName1,searchKeyValue,key, orderby, ordertype)
            ...
            const lisResult = await sqlQuery.searchQueryNoLimit(this.tableName5,searchKeyValue,key, orderby, ordertype)
            if(!lisResult.length) return res.status(400).json({ errors: [ {msg : 'user not found'}]})

            var strDecriptPassword = this.getHashPassword(lisResult[0].encryption_key, lisResult[0].password);
    ...
    updateSecurityPin = async (req, res) => {
        try{
            ...
            console.log('[Admin/updateSecurityPin]', JSON.stringify(req.body), JSON.stringify(req.query));
            var searchKeyValue = {
                username : req.body.user_detials.username,
                status : 1,
                active : 1
            ...
            const lisResponse1 = await sqlQuery.searchQueryNoLimit(this.tableName1,searchKeyValue,key, orderby, ordertype)
            if(!lisResponse1.length) return res.status(400).json({ errors: [ {msg : 'user not found'}]})
            ...
            if(strDecriptPin != req.body.oldPin) return res.status(400).json({ errors: [ {msg : 'Old Pin is In-correct'}]})

            encryptionKey = lisResponse1[0].encryption_key
            let intPin = req.body.newPin

            const strEncriptedPin = this.setHashPassword(encryptionKey,intPin.toString())
            ...
            const { affectedRows, changedRows, info } = ObjResult;
            const message = !affectedRows ? 'user not found' :
                affectedRows && changedRows ? 'user pin updated successfully' : 'Details updated';
    ...
    updateAgentPassword = async (req, res) => {
        try {
            ...
            console.log('[Admin/updateAgentPassword]', JSON.stringify(req.body), JSON.stringify(req.query));
            ...
            const strEncriptedPassword = this.setHashPassword(lisResult[0].encryption_key,req.body.password)
            ...
            const { affectedRows, changedRows, info } = ObjResult;
            const message = !affectedRows ? 'user not found' :
                affectedRows && changedRows ? 'user password updated successfully' : 'Details Updated';
    ...
    addSubAdmin = async(req, res) => {
        try{
            ...
            console.log('[Admin/addSubAdmin]', JSON.stringify(req.body), JSON.stringify(req.query));
            let operator_uuid = '', operatorName = ''
        ...
        //verify departemnt id provided
            ...
            const lisResponce1 = await sqlQueryReplica.searchOrQuery(this.tableName6,searchKeyValue,key, orderby, ordertype, 1, 0)
            if(lisResponce1.length === 0) return res.status(400).json({ errors: [ {msg : 'Department not found'}]})
        ...
        //verify operator_uuid
            if(req.body.operator_uuid){
                ...
                const lisResponse2 = await sqlQueryReplica.searchQuery(this.tableName10, searchKeyValue, key, orderby, ordertype, 1, 0)
            ...
            if(req.body.gender !== null && req.body.gender !== undefined) {
                ...
                if(!lisGender.includes(req.body.gender)) return res.status(400).json({ errors: [ {msg : 'use gender 1 for male, 2 for female, 0 for other'}]})
            ...
            if( lisRegion.length > 0){
                for(let i = 0; i < lisRegion.length; i++)
                {
                    ...
                    if(!region_uuid || !regionName) return res.status(400).json({ errors: [ {msg :'Region parameter are not proper'}]})
                    var responce = await commonQueryCommon.checkRegionwithoutCountry(region_uuid,regionName)
    ...
    getSubAdminList = async(req,res,next) => {
        try {
            ...
            var key = ["full_name AS name","username AS userid","usertype_id","mobile","CAST(created_on AS CHAR(20)) AS createdOn","status"]
            var orderby = "full_name"
            var ordertype = "ASC"
            ...
            if(!lisDepart) return res.status(400).json({ errors: [ {msg : "department list error"}] });
    ...
    updateSubadminDetails = async (req,res,next) =>{
        try{
            ...
            console.log('[Admin/updateSubadminDetials]', JSON.stringify(req.body), JSON.stringify(req.query));
            let operator_uuid = '', operatorName = ''
            ...
            const lisResponce1 = await sqlQueryReplica.searchQuery(this.tableName6,searchKeyValue,key, orderby, ordertype, 1, 0)
            if(lisResponce1.length === 0) return res.status(400).json({ errors: [ {msg : 'Department not found'}]})
        ...
        //verify operator_uuid
            ...
            // console.log(objResult)
            const { affectedRows, changedRows, info } = objResult;
            const SubAdminDetailsMessage = !affectedRows ? 'sub Admin not found' :
                affectedRows && changedRows ? 'subAdmin details updated successfully' : 'Sub Admin details update.';
            
            if(!affectedRows) return res.status(400).json({ errors: [ {msg : 'Sub Admin not found'}]})
            ...
            return res.status(200).send({message : SubAdminDetailsMessage + " and region list updated successfully"})
        }catch (error){
            console.log(error);
            return res.status(400).json({ errors: [ {msg : error.message}] });
        }
    ...
    getPerSubAdminDetails = async(req,res,next)=> {
        ...
        //search sub admin details
            ...
            var key =["username As userid","usertype_id","full_name as name","emailid as email","mobile","gender","city_name AS city","Address AS address","CAST(operator_uuid AS CHAR(16)) AS operator_uuid", "operator_name AS operatorName","region_id"]
            var orderby ="username"
            var ordertype = "ASC"

            var lisUserDetails = await sqlQueryReplica.searchQuery(this.tableName1, searchKeyValue, key, orderby, ordertype, 1, 0)
            if(lisUserDetails.length === 0) return res.status(204).send({ message:'sub admin details not found'})
        ...
        //get the region list
            ...
            var ordertype = "ASC"
            var listRegion = await sqlQueryReplica.searchQueryNoLimit(this.tableName7,searchKeyValue,key, orderby, ordertype)
        ...
        // editing output
            ...
            other.assignRegionList = listRegion.length > 0 ? listRegion : "No region Assigned"
    ...
    changeActiveState = async (req,res) =>{
        try{
            const errors = validationResult(req);
                ...
                console.log('[Admin/ChangeActiveState]', JSON.stringify(req.body), JSON.stringify(req.query));
            //search sub admin active state
                ...
                var lisUserDetails = await sqlQuery.searchQuery(this.tableName1, searchKeyValue, key, orderby, ordertype, 1, 0)
                if(lisUserDetails.length === 0) return res.status(204).send({ message:'sub admin details not found'})
            ...
            // change status value
                ...
                var objResult = await sqlQuery.updateQuery(this.tableName1, param, searchKeyValue)

                await redisMaster.delete(`ADMIN_LOGIN_ATTEMPT_${req.body.subAdminUserid}`)

                const { affectedRows, changedRows, info } = objResult;
                const message = !affectedRows ? 'Sub-Admin not found' :
                affectedRows && changedRows ? 'Sub-Admin status updated successfully' : 'status chage faild';
    ...
    deleteSubAdmin = async (req,res) =>{
        try{
            const errors = validationResult(req);
                ...
                console.log('[Admin/deleteSubAdmin]', JSON.stringify(req.body), JSON.stringify(req.query));
                var searchKeyValue = {
                    username : req.body.subAdminUserid,
                    active : 1
                }
            ...
            // change status value
                ...
                const { affectedRows, changedRows, info } = objResult;
                const message = !affectedRows ? 'Sub-Admin not found' :
                affectedRows && changedRows ? 'Sub-Admin Deleted successfully' : 'Sub-Admin allready Deleted';
    ...
    loginAccess = async(req,res) => {
        try{
            const errors = validationResult(req);
                ...
                console.log('[Admin/loginAccess]', JSON.stringify(req.body), JSON.stringify(req.query));
            //login message
            // console.log(req.body.message)

            // check reason
            const resonCheck = await sqlQueryReplica.searchQuery(this.tableName9,{
            ...
            //search sub admin active state
                ...
                var key =['admin_userid','username',"full_name","usertype","usertype_id",'mobile']
                var orderby ="username"
                var ordertype = "ASC"
            ...
            var data = { 
                ...
                user_uuid : '0000000000000000',
                intCreatedByType : req.body.user_detials.id, 
                intUserType : (req.body.user_detials.type == roles.Admin || req.body.user_detials.type == roles.SubAdmin) ? (req.body.user_detials.type == roles.Admin ? 1 : 2) : (req.body.user_detials.type) ,
                userIpAddress : req.body.userIpAddress ? req.body.userIpAddress : 0, 
                userMacAddress : req.body.userMacAddress ? req.body.userMacAddress : 0, //str
                userOsDetails : req.body.userOsDetails ? req.body.userOsDetails : 0, //str
                userImeiNumber : req.body.userImeiNumber ? req.body.userImeiNumber : 0, //str
                ...
                userAppVersion : req.body.userAppVersion ? req.body.userAppVersion : null, //str
                userApplicationType : req.body.userApplicationType == "Web" ? 1 : req.body.userApplicationType == 'Mobile' ? 2 : 0,
                description : req.body.user_detials.type == roles.Admin || req.body.user_detials.type == roles.SubAdmin ? req.body.user_detials.type + ' Login as subAdmin ' + req.body.reason : 'Agent logedin as subAdmin '+ req.body.reason ,
                userActivityType : 13,
            ...
            // give login access
                const secretKey = process.env.SECRET_JWT || "";
                const token = jwt.sign({ user_id: req.body.subAdminUserid, userType: lisUserDetails[0].usertype }, secretKey, {
                    expiresIn: process.env.SESSION_TIME
                });

                redisFunction.post(`admin_login_session_${lisUserDetails[0].username}`,token)

                // get module permission list
                let listModulePermission = await sqlQueryReplica.searchQuery(this.tableName8,{department_id : lisUserDetails[0].usertype_id},["sub_admin_module_name AS moduleName","perm_view AS permView"],"sub_admin_per_id","ASC",10,0)
                // console.log(listModulePermission)
                if (listModulePermission.length != 10 ) listModulePermission = "permission list is improper"

                res.status(201).send({ userid : req.body.subAdminUserid, token, fullName: lisUserDetails[0].full_name, userType: lisUserDetails[0].usertype, listModulePermission });
    ...
    forgetSendPassword = async(req, res, next) => {
        try{
            // check body and query
                ...
                console.log('[Admin/forgetSendPassword]', JSON.stringify(req.body), JSON.stringify(req.query));
            ...
            // send response for username not found and for wrong mobile number
                if(lisResponce.length === 0) return res.status(400).json({ errors: [ {msg : 'user not found'}] });
                if( !lisResponce[0].mobile || lisResponce[0].mobile.length != 10 ) return res.status(400).json({ errors: [{msg : 'Registered mobile is not proper'}]});
                if(lisResponce[0].status != 1) return res.status(400).json({ errors: [{msg : 'User account is In-Active'}]});
    ...
    getSubAdminPassword = async (req, res) => {
        try{
            // check body and query
                ...
                console.log('[Admin/getSubAdminPassword]', JSON.stringify(req.body), JSON.stringify(req.query));
            // verify admin security pin
                let adminSecurityPin = await sqlQueryReplica.searchQuery(this.tableName1,{ admin_userid: req.body.user_detials.id},['tpin','encryption_key'],'admin_userid','ASC',1, 0)
                if(adminSecurityPin.length == 0) return res.status(400).json({ errors: [ {msg :'Securty pin matching error'}]})

                let decriptSecurityPin = this.getHashPassword(adminSecurityPin[0].encryption_key,adminSecurityPin[0].tpin)
                if(decriptSecurityPin != req.body.securityPin) return res.status(400).json({ errors: [ {msg :'Entered pin is incorrect'}]});

            // get agent security pin and 
                let subAminSecurityPin = await sqlQueryReplica.searchQuery(this.tableName1,{ username: req.body.subAdminUserid,active:1},['password','encryption_key'],'admin_userid','ASC',1, 0)
                if(subAminSecurityPin.length == 0) return res.status(400).json({ errors: [ {msg :'sub-Admin account not found'}]})

                let decriptSecurityPassword = this.getHashPassword(subAminSecurityPin[0].encryption_key,subAminSecurityPin[0].password)
    ...
    updateSubAdminPassword = async (req, res) => {
        try{
            // check body and query
                ...
                console.log('[Admin/updateSubAdminPassword]', JSON.stringify(req.body), JSON.stringify(req.query));
            // verify admin security pin
                let adminSecurityPin = await sqlQuery.searchQuery(this.tableName1,{ admin_userid: req.body.user_detials.id},['tpin','encryption_key'],'admin_userid','ASC',1, 0)
                if(adminSecurityPin.length == 0) return res.status(400).json({ errors: [ {msg :'Securty pin matching error'}]})
            ...
            // get agent security pin and passowrd
                let subAdminSecurity = await sqlQuery.searchQuery(this.tableName1,{ username: req.body.subAdminUserid,active:1},['password','encryption_key','tpin'],'admin_userid','ASC',1, 0)
                if(subAdminSecurity.length == 0) return res.status(400).json({ errors: [ {msg :'Sub-Admin account not found'}]})

                let decriptSecurityPassword = this.getHashPassword(subAdminSecurity[0].encryption_key,subAdminSecurity[0].password)
                decriptSecurityPin = this.getHashPassword(subAdminSecurity[0].encryption_key,subAdminSecurity[0].tpin)

            // update password and security pin with new security key
                let newEncryptionKey = this.setEncryptionKey()
                let newPasswordHash = this.setHashPassword(newEncryptionKey,req.body.newPassword)
                let newPinHash = this.setHashPassword(newEncryptionKey,decriptSecurityPin)
                let param ={
                    encryption_key : newEncryptionKey,
                ...
                let upadteResose = await sqlQuery.updateQuery(this.tableName1, param, {username: req.body.subAdminUserid})
                const { affectedRows, changedRows, info } = upadteResose;
                const message = !affectedRows ? 'Sub-Admin not found' :
                affectedRows && changedRows ? 'Sub-Admin password updated successfully' : 'Sub-Admin password alredy updated';
    ...
    getSubAdminPin = async (req, res) => {
        try{
            // check body and query
                ...
                console.log('[Admin/getSubAdminPin]', JSON.stringify(req.body), JSON.stringify(req.query));
            // verify admin security pin
                let adminSecurityPin = await sqlQueryReplica.searchQuery(this.tableName1,{ admin_userid: req.body.user_detials.id},['tpin','encryption_key'],'admin_userid','ASC',1, 0)
                if(adminSecurityPin.length == 0) return res.status(400).json({ errors: [ {msg :'Securty pin matching error'}]})
            ...
            // get agent security pin and 
                let subAminSecurityPin = await sqlQueryReplica.searchQuery(this.tableName1,{ username: req.body.subAdminUserid,active:1},['tpin','encryption_key'],'admin_userid','ASC',1, 0)
                if(subAminSecurityPin.length == 0) return res.status(400).json({ errors: [ {msg :'Sub-Admin account not found'}]})

                decriptSecurityPin = this.getHashPassword(subAminSecurityPin[0].encryption_key,subAminSecurityPin[0].tpin)
    ...
    updateSubAdminPin = async (req, res) => {
        try{
            // check body and query
                ...
                console.log('[Admin/updateSubAdminPin]', JSON.stringify(req.body), JSON.stringify(req.query));
            // verify admin security pin
                let adminSecurityPin = await sqlQuery.searchQuery(this.tableName1,{ admin_userid: req.body.user_detials.id},['tpin','encryption_key'],'admin_userid','ASC',1, 0)
                if(adminSecurityPin.length == 0) return res.status(400).json({ errors: [ {msg :'Securty pin matching error'}]})
            ...
            // update password and security pin with new security key
                let newEncryptionKey = this.setEncryptionKey()
                let newPasswordHash = this.setHashPassword(newEncryptionKey,decriptSecurityPassword)
                let newPinHash = this.setHashPassword(newEncryptionKey,req.body.newSecurityPin)
                let param ={
                    encryption_key : newEncryptionKey,
                    password : newPasswordHash,
                    tpin : newPinHash
    ...
    async setUserName () {
        ...
        while( i < 10 ){
            ...
            let userCount = await sqlQuery.searchQuery(this.tableName1, {username : userName}, ["COUNT(1)"], 'admin_userid','ASC', 1, 0)
