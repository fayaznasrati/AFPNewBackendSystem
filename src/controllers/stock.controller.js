...
class stockController {
  ...
  getAgentDetials = async (req, res, next) => {
    try {
      ...
      let lisTotalRecords = await stockModule.getUserWithBalanceCount(
        searchKeyValue
      );
      ...
      const lisResult = await stockModule.getUserWithBalance(
        searchKeyValue,
        limit,
        offset
      );
  ...
  downloadAgentDetials = async (req, res, next) => {
    try {
      ...
      const lisTotalRecords = await stockModule.getUserWithBalanceCount(
        searchKeyValue
      );
      ...
      if (req.query.pageNumber == 0 && isAdmin) {
        ...
        setTimeout(() => {
          fs.unlink(filePath, (err) => {
            if (err)
              console.error("Error deleting agent report file:", fileName);
    ...
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({
            errors: [{ msg: "Stock transfer is not allowed for a while" }],
          }); // return {status : 400, message : 'Stock transfer is not allowed for a while.'}
  ...
  processStockTransfer = async (data) => {
    try {
      ...
      const strDate = dtCurrentDate
        .toISOString()
        .slice(0, 19)
        .replace("T", " "); //dt current date time
      const strUniqueNumber = data.transactionId
        ? data.transactionId
        : await dataBaseId(dtCurrentDate); //str unique number

      if (data.reciever_uuid == data.user_uuid)
        return { status: 400, message: "Reciever and sender id cant be same." };
      ...
      const lisResponce4 = await sqlQuery.searchQuery(
        this.tableName4,
        searchKeyValue,
        key,
        orderby,
      ...
      if (lisResponce4[0].user_status != 1)
        return { status: 400, message: "Receiver account is In-Active" };
      // return res.status(400).json({ errors: [ {msg : "reciever account not found"}] });
      // console.log(lisResponce4)
      //assign values of proper variable

      if (data.checkParent == 1 && lisResponce4[0].parent_id != data.userid)
        return {
          status: 400,
          message: "Stock transfer to direct child is only allowed",
        };
      ...
      let checkLastTransaction = await sqlQuery.searchQuery(
        this.tableName5,
        {
      ...
      if (checkLastTransaction[0]["COUNT(1)"] > 0)
        return {
          status: 400,
          message: `Same Transaction done within ${process.env.STOCK_TRANSFER_TIME_LIMIT} min`,
        };
      ...
      if (intRecieverCommissionType == 0)
        return {
          status: 400,
          message: "Agent commission not set, please set commission first",
        };
      // return res.status(400).json({ errors: [ {msg : "Agent commission not set, please set commission first"}] });
      if (intRecieverCommissionType == 1) {
        var commissionVal = await sqlQuery.searchQuery(
          this.tableName7,
          { user_uuid: data.reciever_uuid },
          ["commission_value"],
          "userid",
          "ASC",
          1,
        ...
        commissionVal =
          commissionVal.length > 0 ? commissionVal[0].commission_value : 0;
      }
      let commissionAmt =
        intRecieverCommissionType == 1
          ? (commissionVal / 100) * Number(data.amount)
          : 0;
      ...
      const lisResponce1 = await sqlQuery.searchQueryTran(
        this.tableName1,
        searchKeyValue,
        key,
        orderby,
      ...
      if (lisResponce1[0].userid != 1) {
        if (
          lisResponce1[0].wallet - (Number(data.amount) + commissionAmt) <
          0
        ) {
        ...
        let channelType = channelList.includes(data.source)
          ? data.source
          : "Web";
        let channelLimit = await sqlQuery.searchQuery(
          this.tableName14,
          { userid: lisResponce1[0].userid, channel: channelType },
          ["threshold", "status"],
          "userid",
          "ASC",
          1,
        ...
        if (channelLimit[0].status != 1) {
          ...
          return {
            status: 400,
            message: `Your ${data.source} channel is In-Active.`,
          };
        }

        if (
          Number(channelLimit[0].threshold) >
          Number(lisResponce1[0].wallet) -
            Number(Number(data.amount) + commissionAmt)
        ) {
          var lisresponce = await sqlQuery.specialCMD("rollback");
          await reset(this.tableName1, data.user_uuid);
          await reset(this.tableName1, data.reciever_uuid);
          return {
            status: 400,
            message: `Your ${data.source} channel Threshold limit reached.`,
          };
        }
      }
      ...
      param = {
        ...
        narration:
          data.senderTxnMessage || `Wallet Transfer to ${strRecieverName}`,
        balance_amount:
          lisResponce1[0].wallet - Number(data.amount) - commissionAmt, //db balance
        trans_for: data.senderTxnMessage
          ? "Received rollback amount"
          : "Wallet Transfer",
      };
      //fire sql query
      ...
      // fire sql query to get reciever balance
      const lisResponce2 = await sqlQuery.searchQueryTran(
        this.tableName1,
        searchKeyValue,
        key,
        orderby,
      ...
      const dbReviceBalance =
        lisResponce2.length === 0 ? 0 : lisResponce2[0].wallet;
      var strRecieverId =
        lisResponce2.length === 0 ? 0 : lisResponce2[0].userid;
      ...
      param = {
        ...
        narration:
          data.recieverTxnMessage || `Wallet Received from ${strSenderName}`,
        balance_amount:
          Number(dbReviceBalance) + Number(data.amount) + commissionAmt, //db balance amount
        trans_for: data.recieverTxnMessage
          ? "Transferred Rollback amount"
          : "Wallet Received",
      };
      //fire sql query
      var objResponce = await sqlQuery.createQuery(this.tableName2, param);
      ...
      logData.push({
        ...
        created_by_type:
          data.type === userList.Admin || data.type === userList.SubAdmin
            ? data.id
        ...
        description:
          data.rollbackStatus == 1
            ? `Dear ${
                lisResponce4[0].full_name
              } ${strRecieverName} You have received Rollback Amount: ${parseFloat(
                String(Number(data.amount) + commissionAmt)
              ).toFixed(2)} AFN against: ${
                data.rechangeNumber
              } actual Amount: ${
                data.rechargeAmount
              } AFN, Current Bal: ${parseFloat(
                String(
                  Number(dbReviceBalance) + Number(data.amount) + commissionAmt
                )
              ).toFixed(2)} AFN, Thanks for being Afghan Pay Agent`
            : `Dear ${
                lisResponce4[0].full_name
              } ${strRecieverName}, You received amount ${parseFloat(
                String(Number(data.amount) + commissionAmt)
              ).toFixed(2)} AFN from ${
                data.name
              } ${strSenderName}, Bal: ${parseFloat(
                String(
                  Number(dbReviceBalance) + Number(data.amount) + commissionAmt
                )
              ).toFixed(
                2
              )} AFN TXN ID ${strUniqueNumber}, Thanks for being Afghan Pay agent!`,
      ...
      logData.push({
        ...
        description: `Dear ${
          data.name
        } ${strSenderName}, You transferred amount ${parseFloat(
          String(Number(data.amount) + commissionAmt)
        ).toFixed(2)} AFN to ${lisResponce4[0].full_name} ${
          lisResponce4[0].username
        }, Bal: ${parseFloat(
          String(lisResponce1[0].wallet - Number(data.amount) - commissionAmt)
        ).toFixed(
          2
        )} AFN TXN ID ${strUniqueNumber}, Thanks for being Afghan Pay ${
          data.type === userList.Admin || data.type === userList.SubAdmin
            ? data.type === userList.Admin
              ? "Admin"
              : "Sub-Admin"
            : "Agent"
        }!`,
        activity_type:
          data.type === userList.Admin || data.type === userList.SubAdmin
            ? data.type === userList.Admin
              ? 17
              : 26
            : 24, // 1-Login;2-Change Password;3-Change Profile
      ...
      const intResult = await httpRequestMakerCommon.httpPost(
        "activity-log/multiple",
        { mulActivityLog: logData }
      );
      var strLog =
        intResult == 1
          ? "Agent login log added successfully"
          : intResult == 2
          ? "Agent login log error"
          : "end point not found";
      ...
      if (intResult != 1) {
        ...
        return {
          status: 400,
          message: "Stock transfer Failed as log adding have issue",
        };
      ...
      if (data.type === userList.Admin || data.type === userList.SubAdmin) {
        ...
        switch (String(data.languageType)) {
          case "2": // Pashto
            smsDetails.recieverMessage = `ښاغلی ${data.name}! تاسو په ${
              lisResponce4[0].full_name
            } ${lisResponce4[0].username} حساب کې ${parseFloat(
              String(Number(data.amount) + commissionAmt)
            ).toFixed(2)} افغانۍ اضافه کړې، ستاسو پاتې کریډیټ ${parseFloat(
              String(
                lisResponce1[0].wallet - Number(data.amount) - commissionAmt
              )
            ).toFixed(2)} افغانۍ دی. مننه، افغان پی.`;
            break;
          case "3": // Dari
            smsDetails.recieverMessage = `محترم ${
              ...
              String(Number(data.amount) + commissionAmt)
            ).toFixed(2)} افغانی را به حساب ${lisResponce4[0].full_name} ${
              lisResponce4[0].username
            } اضافه نمودید، کریدیت باقیمانده شما ${parseFloat(
              String(
                lisResponce1[0].wallet - Number(data.amount) - commissionAmt
              )
          ...
          case "1": // english
          default:
            smsDetails.recieverMessage = `Dear ${
              data.name
            } ${strSenderName}, You transferred amount ${parseFloat(
              String(Number(data.amount) + commissionAmt)
            ).toFixed(2)} AFN to ${lisResponce4[0].full_name} ${
      ...
      if (data.rollbackStatus == 1) {
        smsDetails = {
          ...
          recieverMessage: `Dear ${
            ...
            String(Number(data.amount) + commissionAmt)
          ).toFixed(2)} AFN against: ${data.rechangeNumber} actual Amount: ${
            data.rechargeAmount
          } AFN, Current Bal: ${parseFloat(
            String(
              Number(dbReviceBalance) + Number(data.amount) + commissionAmt
        ...
        switch (String(lisResponce4[0].prefer_lang)) {
          case "2": // Pashto
            smsDetails.recieverMessage = `ښاغلی ${
              lisResponce4[0].full_name
            }! تاسو په اندازه ${parseFloat(
              String(Number(data.amount) + commissionAmt)
            ).toFixed(2)} افغانۍ له ${
              data.name
            } څخه ترلاسه کړې. ستاسو د حساب اوسنی کریډیټ ${parseFloat(
              String(
                Number(dbReviceBalance) + Number(data.amount) + commissionAmt
              )
          ...
          case "3": // Dari
            smsDetails.recieverMessage = `محترم ${
              lisResponce4[0].full_name
            }!  شما به مقدار ${parseFloat(
              String(Number(data.amount) + commissionAmt)
            ).toFixed(2)} افغانی از ${
              data.name
            } دریافت نمودید، کریدیت فعلی حساب شما ${parseFloat(
              String(
                Number(dbReviceBalance) + Number(data.amount) + commissionAmt
              )
            ).toFixed(2)} افغانی است. تشکر، افغان پی.`;
      ...
      return {
        status: 200,
        message: "money transfer sucessfully.",
        transactionId: strUniqueNumber,
        recieverNumber: lisResponce4[0].mobile,
        closingBalance:
          lisResponce1[0].wallet - Number(data.amount) - commissionAmt,
      };
    } catch (error) {
  ...
  adminStockTransferReport = async (req, res, next) => {
    try {
      ...
      const lisResult = await stockModule.adminStocksDetials(
        searchKeyValue,
        limit,
        offset
      );
  ...
  downloadAdminStockTransferReport = async (req, res, next) => {
    try {
      ...
      if (req.query.pageNumber == 0) {
        ...
        const FILE_PREFIX = `admin_stock_transfer_report_${QUERY_HASH}_`;
    ...
    try {
      ...
      searchQkeys.forEach((key) => {
        if (key.includes("user_id"))
          req.query.userId = key.slice(7, key.length);
      });
      // user_idAFP-32585
      // console.log(req.query)
      // search parameter
      ...
      const lisResult = await stockModule.agentStocksDetials(
        searchKeyValue,
        limit,
        offset
      );
  ...
  downloadDownlineStockTransferReport = async (req, res, next) => {
    try {
      ...
      if (req.query.pageNumber == 0) {
        ...
        // Add SrNo to each row in the data
        const dataWithSrNo = lisResult.map((row, index) => ({
          SrNo: index + 1,
          ...row,
        }));

        // dynamically create columns (SrNo first, then other columns)
        const allColumns = {
          SrNo: "Sr No",
          ...Object.fromEntries(
            Object.keys(lisResult[0]).map((key) => [key, key])
          ),
        };

        const columns = Object.keys(allColumns).map((key) => ({
          header: allColumns[key],
          key: key,
          width: key === "SrNo" ? 10 : 20, // Smaller width for serial number
        }));

        worksheet.columns = columns;

        // Add all data rows with serial numbers
        dataWithSrNo.forEach((row) => {
        ...
        const totalRowValues = {
          ...
          ...Object.fromEntries(
            Object.keys(lisResult[0]).map((key) => {
              if (key === "Transfar_Amount") return [key, totalTransfarAmount];
              return [key, ""];
            })
          ),
        };

        const totalExcelRow = worksheet.addRow(totalRowValues);
        ...
        totalExcelRow.fill = {
          type: "pattern",
          pattern: "solid",
          fgColor: { argb: "FF203764" }, // Dark blue background
        };

        // Add "TOTAL" label in the first data column (after SrNo)
        const firstDataColumn = Object.keys(lisResult[0])[0];
        totalExcelRow.getCell(firstDataColumn).value = "TOTAL:";

        // Format the amount cell
        totalExcelRow.getCell("Transfar_Amount").numFmt = "#,##0.00";

        // Center align the SrNo column
 ...
 downloadPDFDownlineStockTransferReport = async (req, res, next) => {
  ...
  stocksRequests = async (req, res, next) => {
    try {
      ...
      var lisTotalRecords = await stockModule.getPendingRequestCount(
        searchKeyValue
      );
      ...
      var results = await stockModule.getPendingRequest(
        searchKeyValue,
        limit,
        offset
      );
      if (results.length === 0)
        return res.status(204).send({ message: "no pending requests" });
      ...
      var changeResults = results.map((sinResult) => {
        ...
        other.mode = mode;
        other.rcptUrl = rcptUrl
          ? rcptUrl.includes(awsURL)
            ? rcptUrl.replace(awsURL, "")
            : rcptUrl
          : null;
        return other;
      });
  ...
  downloadPendingStocksRequests = async (req, res, next) => {
    try {
      ...
      const lisTotalRecords = await stockModule.getPendingRequestCount(
        searchKeyValue
      );
      const intTotlaRecords = Number(lisTotalRecords[0].count);
      ...
      const results = await stockModule.getPendingRequest(
        searchKeyValue,
        limit,
        offset
      );
  ...
  if (changeResults.length > 0) {
    ...
    const dataWithSrNo = changeResults.map((row, index) => ({
      SrNo: index + 1,
      ...row,
    }));

    // Create columns with SrNo as first column
    const columns = [
      { header: "Sr No", key: "SrNo", width: 10 },
      ...Object.keys(changeResults[0]).map((key) => ({
        header: key,
        key: key,
        width: key.length < 20 ? 20 : key.length + 5,
      })),
    ...
    headerRow.font = { bold: true, color: { argb: "FFFFFFFF" } }; // White bold text
    headerRow.fill = {
      type: "pattern",
      pattern: "solid",
  ...
  acceptStockRequest = async (req, res, next) => {
    try {
      ...
      const lisResponce3 = await stockModule.getAgentUuid(
        req.body.transferNumber
      );
      if (lisResponce3.length === 0)
        return res
          .status(400)
          .json({ errors: [{ msg: "reference number not found" }] });
      ...
      if (lisResponce4.length == 0)
        return res
          .status(400)
          .json({ errors: [{ msg: "reciever account not found" }] });
      ...
      if (intRecieverCommissionType == 0)
        return res
          ...
          .json({
            errors: [
              { msg: "Agent commission not set, please set commission first" },
            ],
          });
      if (intRecieverCommissionType == 1) {
        var commissionVal = await sqlQuery.searchQuery(
      ...
      commissionVal =
        commissionLengt > 0 ? commissionVal[0].commission_value : 0;
      ...
      if (!affectedRows)
        return res
          .status(400)
          .json({ errors: [{ msg: "sender account not found" }] });
      if (!(affectedRows && changedRows))
        return res
          .status(200)
          .send({ message: "earlier transation under process" });
      ...
      if (!(affectedRows && changedRows)) {
        ...
        return res
          .status(200)
          .send({ message: "reviever earlier transation under process" });
      }
      ...
      const commissionAmt =
        intRecieverCommissionType == 1
          ? (commissionVal / 100) * Number(req.body.amount)
          : 0;
      // balance should be grater then minimum balance
      // console.log(Number(req.body.amount),((commissionVal/100)*Number(req.body.amount)) )
      if (lisResponce1[0].userid != 1) {
        if (
          lisResponce1[0].wallet - (Number(req.body.amount) + commissionAmt) <
          0
        ) {
          await sqlQuery.specialCMD("rollback");
          await reset(this.tableName1, req.body.user_detials.user_uuid);
          ...
          return res
            .status(400)
            .json({ errors: [{ msg: "in sufficient balance" }] });
        }
      }
      //update reciever balanced
      var param = {
      ...
      if (lisResponce2.length === 0) {
        ...
        const lisResponce3 = await sqlQuery.searchQuery(
          this.tableName4,
          searchKeyValue,
          key,
          orderby,
      ...
      param = {
        ...
        trans_type: 1, // type debit
        narration:
          req.body.narration || `Wallet Recieved from ${strSenderName}`,
        balance_amount:
          Number(dbReviceBalance) + Number(req.body.amount) + commissionAmt, //db balance amount
      ...
      param = {
        ...
        trans_type: 2, // type credit
        narration:
          req.body.narration || `Wallet Transfer to ${strRecieverName}`,
        balance_amount:
          lisResponce1[0].wallet - Number(req.body.amount) - commissionAmt, //db balance
      ...
      logData.push({
        ...
        user_type:
          ...
          req.body.user_detials.type == userList.SubAdmin
            ? 0
            : req.body.user_detials.type, // 1- Admin ,2- Member
        created_by_type:
          req.body.user_detials.type === userList.Admin ||
          req.body.user_detials.type === userList.SubAdmin
            ? req.body.user_detials.id
            : req.body.user_detials.userid, // userList
        ip_address: req.body.userIpAddress ? req.body.userIpAddress : 0,
        mac_address: req.body.userMacAddress ? req.body.userMacAddress : 0,
        os_details: req.body.userOsDetails ? req.body.userOsDetails : 0,
        imei_no: req.body.userImeiNumber ? req.body.userImeiNumber : 0,
        ...
        description:
          "Fund transferred to " +
          (req.body.user_detials.type == 0
            ? " Admin - " + lisResponce4[0].username
            : lisResponce4[0].username),
        activity_type:
          req.body.user_detials.type === userList.Admin ||
          req.body.user_detials.type === userList.SubAdmin
  ...
  stockRequestReport = async (req, res) => {
    try {
      ...
      const lisTotalRecords = await stockModule.stockTransferReportCount(
        searchKeyValue
      );
      ...
      const lisResult = await stockModule.stockTransferReport(
        searchKeyValue,
        limit,
        offset
      );
      ...
      var changeResults = lisResult.map((sinResult) => {
        var { rcptUrl, payment_mode, status, usertype_id, ...other } =
          sinResult;
  ...
  dwonloadStockRequestReport = async (req, res) => {
    try {
      ...
      const changeResults = lisResult.map((sinResult) => {
        const { rcptUrl, payment_mode, status, usertype_id, ...other } =
          sinResult;
        return {
          ...other,
          mode: payment_mode === 1 ? "Cash" : "Bank Transfer",
  ...
  rejectStockRequest = async (req, res, next) => {
    try {
      ...
      const message = !affectedRows
        ...
        : "request reject faild, values are same";
  ...
  createStockRequest = async (req, res) => {
    try {
      ...
      if (req.body.paymentMode == 2) {
        ...
        if (!req.body.referenceNumber.match(/^[0-9a-zA-Z]+$/))
          return res
            .status(400)
            .json({ errors: [{ msg: "Reference number should be proper" }] });
      ...
      if (message.includes("Column 'parent_id' cannot be null"))
        return res
          .status(400)
          .json({ errors: [{ msg: "not allowed to add request" }] });
      return res.status(400).json({ errors: [{ msg: error.message }] });
    }
  };
  ...
  topStockRequestReport = async (req, res) => {
    try {
      ...
      const lisTotalRecords = await stockModule.topStockRequestReportCount(
        searchKeyValue
      );
      ...
      // sql query
      const topAgentList = await stockModule.topStockRequestReport(
        searchKeyValue,
        limit,
        offset
      );
  ...
  saveImage = async (req, res) => {
    try {
      ...
      if (!req.file)
        return res
          .status(400)
          .json({ errors: [{ msg: "Image upload faild" }] });
      console.log(
        "stock/saveImage",
        JSON.stringify(req.body),
        JSON.stringify(req.query)
      ...
      if (uploadDetails) {
        ...
        return res
          .status(400)
          .json({ errors: [{ msg: "image uplaod upload faild" }] });
      }
    } catch (error) {
      console.log(error);
      return res.status(400).json({ errors: [{ msg: error.message }] });
  ...
  getRequestStatus = async (req, res) => {
    try {
      ...
      var key = [
        "trans_number AS requestId",
        "request_amt AS amount",
        "CAST(created_on AS CHAR(20)) AS requestDate",
      ...
      var lisTotalRecords = await sqlQueryReplica.searchQueryNoLimit(
        this.tableName6,
        searchKeyValue,
        ["COUNT(1) AS count", "SUM(request_amt) AS totalAmount"],
        orderby,
        ordertype
      );
      ...
      var lisresult = await sqlQueryReplica.searchQuery(
        this.tableName6,
        searchKeyValue,
        key,
        orderby,
...
async function reset(tableName, user_uuid) {
  ...
  if (!(affectedRows && changedRows))
    return { status: 204, send: "Previous transation under process" };
}
