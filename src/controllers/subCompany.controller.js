...
class companyController {
    ...
    createCompany = async (req, res, next) => {
        try {
            ...
            console.log('Company/createSubCompany', JSON.stringify(req.body), JSON.stringify(req.query));
            ...
            const existingCompany = await sqlQuery.searchOrQuery(
                this.tableName1,
                { sub_sub_company_name: param.sub_sub_company_name, allowed_ips: param.allowed_ips },
            ...
            if (existingCompany && existingCompany.length > 0) {
                return res.status(400).json({ errors: [{ msg: "subCompany already exists with these allowed IPs" }] });
            }
            ...
            if (userCompanyExists && userCompanyExists.length > 0) {
                return res.status(400).json({
                    errors: [{ msg: `subCompany already registered under this user: ${param.ers_ers_account_username}` }]
                });
            }
            ...
            return res.status(201).send({ message: "subCompany created successfully!" });
    ...
    editCompany = async (req, res, next) => {
        try {
            ...
            console.log('Company/editSubCompany', JSON.stringify(req.body), JSON.stringify(req.query));

            const { sub_company_name, allowed_ips,status, ers_account_username,ers_account_user_password   } = req.body;
            ...
            if (allowed_ips !== undefined) {
                ...
                if (conflictingIps.length > 0) {
                    return res.status(400).json({
                        errors: [{ msg: `These IPs are already registered by other sub companies: ${conflictingIps.join(', ')}` }]
                    });
                }
                ...
                updateFields.ers_account_username = ers_account_username;
                 ...
                 // 🧱 Optional field
                if (ers_account_user_password !== null && ers_account_user_password !== undefined) {
            ...
            const changes = {};
            if (sub_company_name && sub_company_name !== existingCompany[0].sub_company_name) {
                changes.sub_company_name = {
                    old: existingCompany[0].sub_company_name,
                    new: sub_company_name
                };
            }
              if (ers_account_username && ers_account_username !== existingCompany[0].ers_account_username) {
                changes.ers_account_username = {
                    old: existingCompany[0].ers_account_username,
                    new: ers_account_username
                };
            }
              if (ers_account_user_password && ers_account_user_password !== existingCompany[0].ers_account_user_password) {
    ...
    getCompanyById = async(req, res, next) => {
           ...
           try {
            ...
            console.log('company/getsubCompanyById', JSON.stringify(req.body), JSON.stringify(req.query))

            //test 
            var key = ['sub_company_id AS id','sub_company_name AS name','allowed_ips','sub_company_api_key AS API_key','encrypted_secret','active AS status','ers_account_username AS ers_account_username','ers_account_user_password AS ers_account_user_password ','created_at','created_by','last_modified_by','last_modified_on' ]
                 if(req.params.id){
                var searchKeyValue = {
    ...
    getCompanies = async (req, res) => {
      try {
          ...
          console.log('subCompanies',JSON.stringify(req.body), JSON.stringify(req.query))
          if (!req.query.pageNumber) req.query.pageNumber = 0

               var orderby = "sub_company_id"
                var ordertype = "DESC"
                
                var key = ['sub_company_id','sub_company_name ','allowed_ips','sub_company_api_key','bcrypt_hash','active','ers_account_username ','created_at','created_by','last_modified_by','last_modified_on' ]
...
;
          let companyList = await sqlQuery.searchQueryNoCon(this.tableName1,  ['sub_company_id AS id','sub_company_name AS name','allowed_ips','sub_company_api_key AS API_key','bcrypt_hash','active AS status','ers_account_username AS ers_account_username','ers_account_user_password','ers_account_user_password ','created_at','created_by','last_modified_by','last_modified_on' ], orderby, ordertype, limit, offset)
            ...
            const enrichedCompanyList = await Promise.all(
                companyList.map(async (company) => {
                    try {
                        ...
                        console.error(`Error for sub company ${company.name}:`, err.message);
    ...
    try {
        ...
        const key = [
            'sub_company_id', 'sub_company_name', 'allowed_ips', 'sub_company_api_key',
            'bcrypt_hash', 'active', 'ers_account_username', 'ers_account_username',
  ...
  //################---Generate API Key for companies---################
    ...
    try {
        ...
        const result = await sqlQuery.updateQuery(this.tableName1, updateValues, { sub_company_id: companyId });
        if (!result || result.affectedRows === 0) {
   ...
   logActivity = async ({
    ...
    CompanySinglerecharge = async (req, res, next) => {
            ...
            console.log('sub company recharge/singleRecharge', JSON.stringify(req.body), JSON.stringify(req.query))
            ...
            let operator_uuid = '', operator_name = ''
            ...
            if (!operator_uuid && !operator_name ) {
               ...
               let params = {
                    ...
                    sub_company_transaction_id: req.body.transaction_id ? req.body.transaction_id : null,
                    type_id: 1, // top up
                    type_name: 'Top Up',
        ...
        try {
            ...
            return await ERS_Util.ersPost(username, password, `/recharge/single/?username=${username}`, body);
        } catch (error) {
    ...
    //     const searchUser = {
    //         username: company.ers_account_username // Use the company account username to fetch user details
    //     };
    ...
    // }
     ...
     getCompanyActivity = async (req)=> {
        try {
            ...
            const data = await ERS_Util.ersGet(username,password,`/dashboard/agent/dashboard-status/?username=${username}`);
