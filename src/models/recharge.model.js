...
class rechargeModule { 
    ...
    topUpSummeryReport = async (searchKeyValue, userIds) => {
        ...
        let result = await dbConnectionReplica.query(sql,[...sevalues,...sevalues,...sevalues,...sevalues,...sevalues]);
        return result[0]
    }
    ...
    agentTelcoTopUpreport = async (searchKeyValue) =>{ 
        ...
        const { seColumnSet, sevalues } = this.multipleAndColumnSet(searchKeyValue,this.tableName5,this.tableName1) 
    ...
    getCommissionRollbackList = async (rechargeId, operatorId) => {
        ...
        const result = await dbConnectionReplica.query(sql, [rechargeId,operatorId]);
        // console.log(result);
        return result
    }
    ...
    multipleAndColumnSet = (object,tableName1,tableName2) => {
        ...
        let { region_ids, child_ids, between, isIn, request_mobile_no, ...other } = object
        ...
        if( request_mobile_no ) seColumnSet.push( `${tableName2}.request_mobile_no IN ( ${request_mobile_no[0]},${request_mobile_no[1]} ) ` )
        ...
        if( between ) seColumnSet.push(` date( ${tableName2}.${between.key}) BETWEEN '${between.value[0]}' AND '${between.value[1]}' `)
        if ( isIn ) seColumnSet.push( `${tableName2}.${isIn.key} IN ( ${isIn.value} ) ` )
