...
class agentModule { 
            ...
            for(let i = 0; i < result.length; i++){
                ...
                while(parentId){
                    ...
                    if(parentDetails.length > 0){
                        ...
                        let childIds = parentDetails[0].child_id == '' ? `${userId}` :  parentDetails[0].child_id + `,${userId}`
    ...
    multipleAndColumnSet = (object,tableName1,tableName2) => {
        ...
        var seColumnSet = keys.map(key => {
            ...
            if (key == 'trans_date_time') return ` ? <=  date( ${tableName1}.trans_date_time )`
            if (key == 'start_trans_date_time') return ` ? <= date( ${tableName1}.trans_date_time )`
            if (key == 'end_trans_date_time') return `date( ${tableName1}.trans_date_time ) <= ?`
